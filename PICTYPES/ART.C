...
sCanvas *		Art_ToCanvas( sArt * apArt )
{
	...
	Canvas_PaletteFromST(   &lPal[ 0 ],  16, apArt->mPalettes[ 0 ] );
	lpCanvas = Canvas_ImageFrom4Plane( 320, 200, apArt->mPixels, &lPal[ 0 ] );
...
sArt *		Art_FromCanvas( sCanvas * apCanvas, uCanvasPixel * apPalette )
{
	...
	if( lpCanvasIC )
	{
		lpArt = (sArt*)mMEMCALLOC( sizeof(sArt) );

		if( lpArt )
		{
			CanvasIC_PaletteToST( lpCanvasIC, 16, lpArt->mPalettes[ 0 ] );
			CanvasIC_To4Plane(    lpCanvasIC, 320, 200, lpArt->mPixels );
		}
