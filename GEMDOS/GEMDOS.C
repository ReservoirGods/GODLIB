...
/*-----------------------------------------------------------------------------------*
* FUNCTION : GemDos_Psignal( const S16 aSignal,fGemDosPsignal afPsignal )
...
S32	GemDos_Psignal( const S16 aSignal,fGemDosPsignal afPsignal )
{
	U32	lP;
	lP = (U32)afPsignal;
	return( GemDos_Call_WP( eGEMDOS_PSIGNAL, aSignal, (void*)lP ) );
}
...
void *	GemDos_Super( void * apStack )
{
	...
	lpRes = (void*)GemDos_Call_P( eGEMDOS_SUPER, apStack );
	if( !apStack )
	{
		gpGemDosStackUser = lpRes;
	}
	return( lpRes );
}
