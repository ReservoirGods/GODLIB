...
void	Except_ShowRegisters( sExceptInfo * apInfo)
{
	...
	for( i=0; i<8; i++ )
	{
		sprintf( lString, "D%d: %08lX    A%d: %08lX", i, apInfo->mDataRegs[ i ], i, apInfo->mAddressRegs[ i ] );
		Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
		lY += 8;
	}
	sprintf( lString, "SR   : %04X", apInfo->mSR );
	Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
	lY += 8;

	sprintf( lString, "CRASH: %s", gExceptStrings[ apInfo->mExceptType & 63 ] );
	Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
	lY += 8;

	sprintf( lString, "PC   : %08lX", apInfo->mPC );
	Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
	lY += 8;

	sprintf( lString, "TEXT : %08lX", (U32)lText );
	Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
	lY += 8;

	sprintf( lString, "OFF  : %08lX", apInfo->mPC - (U32)lText );
	Font8x8_Print( lString, Except_GetpPhysic(), 0, lY );
	lY += 8;
	...
	lpStack = (U8*)apInfo->mAddressRegs[ 7 ];
	for( i=0; i<10; i++ )
	{
